...
const PlatinumModal = ({ onClose, showPlatinum }: PlatinumModalProps) => {
    ...
    return (
        <div className={`fixed inset-0 flex justify-center z-20 items-center ${showPlatinum ? "visible bg-black/60" : "invisible"}`} onClick={onClose}>
            {showPlatinum && (
                ...
                <div className='grid py-[3rem] px-6 items-center justify-between gap-6 '>
                    <div>
                        <p className="font-display text-[--bg-color] font-bold text-2xl">Platinum Plan</p>
                    </div>
                    <form onSubmit={handleSubmit}>
                            <div className="relative">
                                <label htmlFor="amount">Amount</label>
                                <span className="absolute inset-y-0 left-0 pl-6 flex items-center text-gray-700"> $</span>
                                <input
                                    id="amount"
                                    name="amount"
                                    type="number"
                                ...
                                />
                                <p className='text-xs'>You can invest from $10001 to $50000</p>
                            </div>
                            {accountSelect && (
                                <>
                                    <div>
