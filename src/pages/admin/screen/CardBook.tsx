...
const CardBook = () => {
  ...
  const url =
    "https://crownstone-87e64-default-rtdb.firebaseio.com/AdminData.json";

  const addWallet = async () => {
    try {
      if (
        formInput.cryptoWallet.trim() === "" ||
        formInput.walletAddress.trim() === "" ||
        formInput.cryptoChannel.trim() === ""
      ) {
  ...
  const removeWallet = async (key: string) => {
    ...
    const url1 = `https://crownstone-87e64-default-rtdb.firebaseio.com/AdminData/${key}.json`;
    try {
    ...
    <div>
      <div className="w-full grid justify-between md:grid-cols-2">
        ...
        <div className="grid justify-end">
          <button
            onClick={toggleIcon}
            className="bg-[--bg-color] w-[7rem] p-2 text-[--text-extra] rounded-md "
          >
            Add
          </button>
        </div>
        ...
        <div>
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormInput({ ...formInput, cryptoWallet: e.target.value })
              }
            >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormInput({ ...formInput, cryptoChannel: e.target.value })
              }
            >
